    ...
    <div class="container">
        ...
        <form method="POST" action="{{route('municipios.update', ['municipio'=>$municipio->muni_codi]) }}">
            ...
            <div class="mb-3">
                ...
                <input type="text" Required class="form-control" id="name" placeholder="Nombre Municipio"
                name="name" value="{{$municipio->muni_nomb}}">
            </div>

            <label for="departament">Departamento:</label>
            <select class="form-select" id="departament" name="code" required >
                <Option selected disabled valude="">Selecciona...</Option>
                @foreach ($departamentos as $departamento)
                <Option value="{{$departamento->depa_codi}}">{{$departamento->depa_nomb}}</Option>
                @endforeach
            </select>
            <div class="mt-3">
                <button type="submit" class="btn btn-primary" id="exampleCheck1">Actualizar</button>
